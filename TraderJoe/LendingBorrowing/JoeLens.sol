    ...
    Exponential
{
     ...
     */
    function addToMarketInternal(JToken jToken, address borrower)
        internal
        returns (Error)
    {
        ...
        if (marketToJoin.version == Version.COLLATERALCAP) {
            ...
            JCollateralCapErc20Interface(address(jToken)).registerCollateral(
                borrower
            );
        }
    ...
    function exitMarket(address jTokenAddress) external returns (uint256) {
        ...
        (uint256 oErr, uint256 tokensHeld, uint256 amountOwed, ) = jToken
        ...
        if (marketToExit.version == Version.COLLATERALCAP) {
            JCollateralCapErc20Interface(jTokenAddress).unregisterCollateral(
                msg.sender
            );
        }
    ...
    function liquidateBorrowAllowed(
        ...
        if (
            !isMarketListed(jTokenBorrowed) || !isMarketListed(jTokenCollateral)
        ) {
            return uint256(Error.MARKET_NOT_LISTED);
        }
        ...
        uint256 borrowBalance = JToken(jTokenBorrowed).borrowBalanceStored(
            borrower
        );
    ...
    function seizeAllowed(
        ...
        if (
            !isMarketListed(jTokenCollateral) || !isMarketListed(jTokenBorrowed)
        ) {
            return uint256(Error.MARKET_NOT_LISTED);
        }

        if (
            JToken(jTokenCollateral).joetroller() !=
            JToken(jTokenBorrowed).joetroller()
        ) {
        ...
        )
    {
        (
            ...
            uint256 liquidity,
            uint256 shortfall
        ) = getHypotheticalAccountLiquidityInternal(account, JToken(0), 0, 0);
    ...
    function liquidateCalculateSeizeTokens(
         ...
         */
        uint256 exchangeRateMantissa = JToken(jTokenCollateral)
    ...
    function _setCollateralFactor(
        ...
        if (
            ...
            oracle.getUnderlyingPrice(jToken) == 0
        ) {
            return
                fail(
                    Error.PRICE_ERROR,
     ...
     */
    function _supportMarket(JToken jToken, Version version)
        external
        returns (uint256)
    {
    ...
    function _setFlashloanPaused(JToken jToken, bool state)
        public
        returns (bool)
    {
        require(
    ...
    function _setCreditLimit(address protocol, uint256 creditLimit) public {
        require(
            msg.sender == admin,
            "only admin can set protocol credit limit"
        );
    ...
    function claimReward(
        ...
        RewardDistributor(rewardDistributor).claimReward(
            rewardType,
            holder,
            jTokens
        );
...
contract JoeLens is Exponential {
    ...
    function jTokenMetadataAll(JToken[] calldata jTokens)
        external
        returns (JTokenMetadata[] memory)
    {
        ...
        for (uint256 i = 0; i < jTokenCount; i++) {
            require(
                address(joetroller) == address(jTokens[i].joetroller()),
                "mismatch joetroller"
            );
            res[i] = jTokenMetadataInternal(
                jTokens[i],
                joetroller,
                priceOracle
            );
    ...
    function jTokenMetadata(JToken jToken)
        public
        returns (JTokenMetadata memory)
    {
    ...
    function jTokenMetadataInternal(
        ...
        (
            ...
            uint256 collateralFactorMantissa,
            JoetrollerV1Storage.Version version
        ) = joetroller.markets(address(jToken));
        address underlyingAssetAddress;
        uint256 underlyingDecimals;
        uint256 collateralCap;
        uint256 totalCollateralTokens;
        ...
        if (version == JoetrollerV1Storage.Version.COLLATERALCAP) {
            collateralCap = JCollateralCapErc20Interface(address(jToken))
                .collateralCap();
            totalCollateralTokens = JCollateralCapErc20Interface(
                address(jToken)
    ...
    function jTokenBalances(JToken jToken, address account)
        public
        returns (JTokenBalances memory)
    {
        ...
        if (compareStrings(jToken.symbol(), nativeSymbol)) {
            ...
            vars.underlyingTokenAllowance = underlying.allowance(
                account,
                address(jToken)
            );
        }
        ...
        (, uint256 collateralFactorMantissa, ) = joetroller.markets(
            address(jToken)
        );

        Exp memory supplyValueInUnderlying = Exp({
            mantissa: vars.balanceOfUnderlyingCurrent
        });
        vars.supplyValueUSD = mul_ScalarTruncate(
            supplyValueInUnderlying,
            underlyingPrice
        );

        Exp memory collateralFactor = Exp({mantissa: collateralFactorMantissa});
        vars.collateralValueUSD = mul_ScalarTruncate(
            collateralFactor,
            vars.supplyValueUSD
        );

        Exp memory borrowBalance = Exp({mantissa: vars.borrowBalanceCurrent});
        vars.borrowValueUSD = mul_ScalarTruncate(
            borrowBalance,
            underlyingPrice
        );
    ...
    function getAccountLimits(Joetroller joetroller, address account)
        public
        returns (AccountLimits memory)
    {
        ...
        (errorCode, vars.liquidity, vars.shortfall) = joetroller
            .getAccountLiquidity(account);
        require(errorCode == 0, "Can't get account liquidity");

        vars.markets = joetroller.getAssetsIn(account);
        JTokenBalances[] memory jTokenBalancesList = jTokenBalancesAll(
            vars.markets,
            account
        );
        for (uint256 i = 0; i < jTokenBalancesList.length; i++) {
            vars.totalCollateralValueUSD = add_(
                vars.totalCollateralValueUSD,
                jTokenBalancesList[i].collateralValueUSD
            );
            vars.totalBorrowValueUSD = add_(
                vars.totalBorrowValueUSD,
                jTokenBalancesList[i].borrowValueUSD
            );
        }

        Exp memory totalBorrows = Exp({mantissa: vars.totalBorrowValueUSD});

        vars.healthFactor = vars.totalCollateralValueUSD == 0
            ? 0
            : vars.totalBorrowValueUSD > 0

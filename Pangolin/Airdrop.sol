...
contract Airdrop {
    ...
    uint256 public constant TOTAL_AIRDROP_SUPPLY = 26_000_000e18;

    // Events
    event ClaimingAllowed();
    event ClaimingOver();
    event PngClaimed(address claimer, uint256 amount);
    ...
    function allowClaiming() external {
        require(
            IPNG(png).balanceOf(address(this)) >= TOTAL_AIRDROP_SUPPLY,
    ...
    function endClaiming() external {
        ...
        // Transfer remainder
        uint256 amount = IPNG(png).balanceOf(address(this));
        require(
    ...
    function claim() external {
        ...
        require(
            IUni(uni).balanceOf(msg.sender) >= oneToken ||
                ISushi(sushi).balanceOf(msg.sender) >= oneToken,
    ...
    function whitelistAddress(address addr, uint96 pngOut) public {
        ...
        require(
            totalAllocated <= TOTAL_AIRDROP_SUPPLY,

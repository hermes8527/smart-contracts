...
contract StakingRewards is ReentrancyGuard, Ownable {
    ...
    function stakeWithPermit(
        ...
        // permit
        IPangolinERC20(address(stakingToken)).permit(
            msg.sender,
            address(this),
            amount,
            deadline,
...
contract LiquidityPoolManagerV2 is Ownable, ReentrancyGuard {
    ...
    uint256 public unallocatedPng = 0;
     ...
     */
    function activateFeeSplit(uint256 avaxSplit_, uint256 pngSplit_)
        external
        onlyOwner
    {
        require(
            avaxSplit_.add(pngSplit_) == 100,
            "LiquidityPoolManager::activateFeeSplit: Split doesn't add to 100"
        );
        require(
            !(avaxSplit_ == 100 || pngSplit_ == 100),
    ...
    function getAvaxLiquidity(address pair) public view returns (uint256) {
        (uint256 reserve0, uint256 reserve1, ) = IPangolinPair(pair)
            .getReserves();
        ...
        if (IPangolinPair(pair).token0() == wavax) {
            ...
            require(
                IPangolinPair(pair).token1() == wavax,
     ...
     * Returns: the amount of liquidity in the pool in units of AVAX
     */
    function getPngLiquidity(address pair, uint256 conversionFactor)
        public
        view
        returns (uint256)
    {
     ...
     */
    function getAvaxPngRatio() public view returns (uint256 conversionFactor) {
        require(
            !(avaxPngPair == address(0)),
            "LiquidityPoolManager::getAvaxPngRatio: No AVAX-PNG pair set"
        );
        (uint256 reserve0, uint256 reserve1, ) = IPangolinPair(avaxPngPair)
            .getReserves();
    ...
    function calculateReturns() public {
        ...
        if (pngPairs.length() > 0) {
            uint256 conversionRatio = getAvaxPngRatio();
            for (uint256 i = 0; i < pngPairs.length(); i++) {
                address pair = pngPairs.at(i);
                uint256 pairLiquidity = getPngLiquidity(pair, conversionRatio);
                uint256 weightedLiquidity = pairLiquidity.mul(weights[pair]);
        ...
        if (splitPools) {
            uint256 avaxAllocatedPng = unallocatedPng.mul(avaxSplit).div(100);
            uint256 pngAllocatedPng = unallocatedPng.sub(avaxAllocatedPng);

            for (uint256 i = 0; i < avaxPairs.length(); i++) {
                uint256 pairTokens = distribution[i].mul(avaxAllocatedPng).div(
                    avaxLiquidity
                );
            ...
            uint256 totalLiquidity = avaxLiquidity.add(pngLiquidity);

            for (uint256 i = 0; i < distribution.length; i++) {
                uint256 pairTokens = distribution[i].mul(unallocatedPng).div(
                    totalLiquidity
                );
                distribution[i] = pairTokens;
                transferred = transferred.add(pairTokens);
    ...
    function distributeTokens() public nonReentrant {
        ...
        for (uint256 i = 0; i < distribution.length; i++) {
            ...
            if (rewardTokens > 0) {
                require(
                    IPNG(png).transfer(stakeContract, rewardTokens),
    ...
    function vestAllocation() external nonReentrant {
        ...
        uint256 actualBalance = IPNG(png).balanceOf(address(this));
        require(
            actualBalance >= unallocatedPng,
